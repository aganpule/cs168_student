"""
Tests that packets take the lowest-cost path even if they are directly connect to a host.
...
def launch():
    ...
    def test_tasklet():
        ...
        api.userlog.debug('Disconnecting s2 and c1')
        s2.unlinkTo(c1)
        ...
        if c1.pings == 1 and c2.pings == 1:
            api.userlog.debug('The ping took the right path')
            good = True
        elif c2.pings == 2 and c1.pings == 0:
            api.userlog.error('The ping took the wrong path')
            good = False
        else:
            api.userlog.error('Something strange happened to the ping')
